...
public class ApidbFileReplicatorTest extends AbstractDataTest {
	...
	/**
	 * A basic test loading an osm file into an API database and verifying that it
	...
	public void testLoadAndDump() throws IOException {
		...
		Osmosis.run(new String[] { "-q", "--replicate-apidb-0.6", "authFile=" + authFile.getPath(),
		...
		Osmosis.run(new String[] { "-q", "--replicate-apidb-0.6", "authFile=" + authFile.getPath(),
				"allowIncorrectSchemaVersion=true", "iterations=2", "--write-replication",
				"workingDirectory=" + workingDirectory.getPath() });

		// Decompress the result file.
		Osmosis.run(new String[] { "-q", "--read-xml-change-0.6",
				new File(workingDirectory, "000/000/002.osc.gz").getPath(), "--write-xml-change-0.6",
				outputFile.getPath() });
