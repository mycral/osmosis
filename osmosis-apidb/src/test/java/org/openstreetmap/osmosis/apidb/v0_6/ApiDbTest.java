...
public class ApiDbTest extends AbstractDataTest {
	...
	/**
	 * A basic test loading an osm file into a mysql database, then dumping it again
	...
	public void testLoadAndDump() throws IOException {
		...
		Osmosis.run(new String[] { "-q", "--read-xml-0.6", inputFile.getPath(), "--write-apidb-0.6",
				"authFile=" + authFile.getPath(), "allowIncorrectSchemaVersion=true" });

		// Dump the database to an osm file.
		Osmosis.run(new String[] { "-q", "--read-apidb-0.6", "authFile=" + authFile.getPath(),
				"allowIncorrectSchemaVersion=true", "--tag-sort-0.6", "--write-xml-0.6", outputFile.getPath() });
	...
	/**
	 * A basic test loading an osm file into a apidb database, then dumping it from
	 * current tables and verifying that it is identical.
	 * 
	 * @throws IOException
	 *             if any file operations fail.
	 */
	...
	/**
	 * A test loading an osm file into a apidb database, then applying a changeset,
	 * then dumping it again and verifying the output is as expected.
	 * 
	 * @throws IOException
	 *             if any file operations fail.
	 */
	...
	public void testApplyChangeset() throws IOException {
		...
		// Dump the database to an osm file.
		Osmosis.run(new String[] { "-q", "--read-apidb-0.6", "authFile=" + authFile.getPath(),
				"allowIncorrectSchemaVersion=true", "--tag-sort-0.6", "--write-xml-0.6", actualResultFile.getPath() });
	...
	/**
	 * A test loading an osm file into a apidb database, then applying a changeset,
	 * then dumping the original snapshot timeframe and verifying the output is as
	 * expected.
	 * 
	 * @throws IOException
	 *             if any file operations fail.
	...
	public void testSnapshotDump() throws IOException, ParseException {
		...
		Osmosis.run(new String[] { "-q", "--read-apidb-0.6",
				"snapshotInstant=" + convertUTCTimeToLocalTime("2008-01-03_00:00:00"), "authFile=" + authFile.getPath(),
				"allowIncorrectSchemaVersion=true", "--tag-sort-0.6", "--write-xml-0.6", actualResultFile.getPath() });
	...
	/**
	 * A test loading an osm file into a apidb database, then applying a changeset,
	 * then extracting the changeset timeframe and verifying the output is as
	 * expected.
	 * 
	 * @throws IOException
	 *             if any file operations fail.
	...
	public void testChangesetDump() throws IOException, ParseException {
		...
		Osmosis.run(new String[] { "-q", "--read-apidb-change-0.6",
				...
				"allowIncorrectSchemaVersion=true", "--write-xml-change-0.6", actualResultFile.getPath() });
