...
public class ApidbChangeWriterFactory extends DatabaseTaskManagerFactory {
	...
	protected TaskManager createTaskManagerImpl(TaskConfiguration taskConfig) {
		...
		populateCurrentTables = getBooleanArgument(taskConfig, ARG_POPULATE_CURRENT_TABLES,
				DEFAULT_POPULATE_CURRENT_TABLES);

		return new ChangeSinkManager(taskConfig.getId(),
				new ApidbChangeWriter(loginCredentials, preferences, populateCurrentTables), taskConfig.getPipeArgs());
	}
}
