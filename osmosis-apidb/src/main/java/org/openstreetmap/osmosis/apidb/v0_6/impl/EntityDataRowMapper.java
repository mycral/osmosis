...
public class EntityDataRowMapper implements RowCallbackHandler {
	 ...
	 * @param readAllUsers
	 *            If true, even anonymous users will be returned. Should be false in
	 *            most cases.
	 */
	...
	/**
	 * Determines the appropriate user name to add to an entity based upon the user
	 * details provided.
	 * 
	 * @param dataPublic
	...
	public void processRow(ResultSet resultSet) throws SQLException {
		...
		user = readUserField(resultSet.getBoolean("data_public"), resultSet.getInt("user_id"),
				resultSet.getString("display_name"));
		changesetId = resultSet.getLong("changeset_id");

		// node = new Node(id, version, timestamp, user, changesetId, latitude,
		// longitude);
