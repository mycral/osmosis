...
public class WayDao extends EntityDao<Way> {
	...
	private ReleasableIterator<DbFeatureHistory<DbOrderedFeature<WayNode>>> getWayNodeHistory(
		...
		FileBasedSort<DbFeatureHistory<DbOrderedFeature<WayNode>>> sortingStore = new FileBasedSort<DbFeatureHistory<DbOrderedFeature<WayNode>>>(
		...
		try {
			...
			sql = "SELECT wn.way_id AS id, wn.node_id, wn.version, wn.sequence_id" + " FROM " + "way_nodes wn"
					+ " INNER JOIN " + selectedEntityStatement
			...
			storeListener = new SortingStoreRowMapperListener<DbFeatureHistory<DbOrderedFeature<WayNode>>>(
					sortingStore);
			...
			resultIterator = new StoreReleasingIterator<DbFeatureHistory<DbOrderedFeature<WayNode>>>(
					sortingStore.iterate(), sortingStore);
	 ...
	 */
	@Override
	protected List<FeatureHistoryPopulator<Way, ?, ?>> getFeatureHistoryPopulators(String selectedEntityStatement,
		...
		// Wrap the way node source into a feature history populator that can attach
		// them to their
		// owning ways.
		featurePopulators.add(new FeatureHistoryPopulator<Way, WayNode, DbOrderedFeature<WayNode>>(wayNodeIterator,
				new WayNodeCollectionLoader()));
