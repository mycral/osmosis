...
public class ChangeWriter implements Completable {
	...
	private static final String INSERT_SQL_NODE = "INSERT INTO nodes (node_id, version, timestamp, visible, changeset_id, latitude, longitude, tile)"
			+ " VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
	...
	private static final String INSERT_SQL_RELATION_MEMBER_MYSQL = "INSERT INTO relation_members (relation_id, version, member_type, member_id, member_role, sequence_id)"
			+ " VALUES (?, ?, ?, ?, ?, ?)";

	private static final String INSERT_SQL_RELATION_MEMBER_PGSQL = "INSERT INTO relation_members (relation_id, version, member_type, member_id, member_role, sequence_id)"
			+ " VALUES (?, ?, ?::nwr_enum, ?, ?, ?)";

	private static final String DELETE_SQL_RELATION_MEMBER = "DELETE FROM relation_members WHERE relation_id = ? AND version = ?";

	private static final String INSERT_SQL_RELATION_MEMBER_CURRENT_MYSQL = "INSERT INTO current_relation_members (relation_id, member_type, member_id, member_role, sequence_id)"
			+ " VALUES (?, ?, ?, ?, ?)";

	private static final String INSERT_SQL_RELATION_MEMBER_CURRENT_PGSQL = "INSERT INTO current_relation_members (relation_id, member_type, member_id, member_role, sequence_id)"
			+ " VALUES (?, ?::nwr_enum, ?, ?, ?)";
	...
	public void write(Node node, ChangeAction action) {
		...
		if (insertNodeStatement == null) {
			...
			selectNodeCurrentCountStatement = statementContainer
					.add(dbCtx.prepareStatement(SELECT_SQL_NODE_CURRENT_COUNT));
		...
		// Update the node if it already exists in the history table, otherwise insert
		// it.
		try {
			exists = checkIfEntityHistoryExists(selectNodeCountStatement, node.getId(), node.getVersion());

		} catch (SQLException e) {
			throw new OsmosisRuntimeException("Unable to check if current node with id=" + node.getId() + " exists.",
					e);
		}
		if (exists) {
			...
			try {
				...
				updateNodeStatement.setInt(prmIndex++,
						FixedPrecisionCoordinateConvertor.convertToFixed(node.getLatitude()));
				updateNodeStatement.setInt(prmIndex++,
						FixedPrecisionCoordinateConvertor.convertToFixed(node.getLongitude()));
				updateNodeStatement.setLong(prmIndex++,
						tileCalculator.calculateTile(node.getLatitude(), node.getLongitude()));
			...
			try {
				...
				insertNodeStatement.setInt(prmIndex++,
						FixedPrecisionCoordinateConvertor.convertToFixed(node.getLatitude()));
				insertNodeStatement.setInt(prmIndex++,
						FixedPrecisionCoordinateConvertor.convertToFixed(node.getLongitude()));
				insertNodeStatement.setLong(prmIndex++,
						tileCalculator.calculateTile(node.getLatitude(), node.getLongitude()));
		...
		if (populateCurrentTables) {
			...
			try {
				...
				throw new OsmosisRuntimeException("Unable to delete current node tags with id=" + node.getId() + ".",
						e);
			}

			// Update the node if it already exists in the current table, otherwise insert
			// it.
			try {
			...
			if (exists) {
				...
				try {
					...
					updateNodeCurrentStatement.setInt(prmIndex++,
							FixedPrecisionCoordinateConvertor.convertToFixed(node.getLatitude()));
					updateNodeCurrentStatement.setInt(prmIndex++,
							FixedPrecisionCoordinateConvertor.convertToFixed(node.getLongitude()));
					updateNodeCurrentStatement.setLong(prmIndex++,
							tileCalculator.calculateTile(node.getLatitude(), node.getLongitude()));
				...
				try {
					...
					insertNodeCurrentStatement.setInt(prmIndex++,
							FixedPrecisionCoordinateConvertor.convertToFixed(node.getLatitude()));
					insertNodeCurrentStatement.setInt(prmIndex++,
							FixedPrecisionCoordinateConvertor.convertToFixed(node.getLongitude()));
					insertNodeCurrentStatement.setLong(prmIndex++,
							tileCalculator.calculateTile(node.getLatitude(), node.getLongitude()));
	...
	public void write(Way way, ChangeAction action) {
		...
		if (insertWayStatement == null) {
			...
			selectWayCurrentCountStatement = statementContainer
					.add(dbCtx.prepareStatement(SELECT_SQL_WAY_CURRENT_COUNT));
		...
		try {
			...
			throw new OsmosisRuntimeException("Unable to delete way history nodes for way with id=" + way.getId() + ".",
					e);
		}

		// Update the way if it already exists in the history table, otherwise insert
		// it.
		try {
	...
	public void write(Relation relation, ChangeAction action) {
		...
		if (insertRelationStatement == null) {
			...
			selectRelationCurrentCountStatement = statementContainer
					.add(dbCtx.prepareStatement(SELECT_SQL_RELATION_CURRENT_COUNT));
			...
			insertRelationTagCurrentStatement = statementContainer
					.add(dbCtx.prepareStatement(INSERT_SQL_RELATION_TAG_CURRENT));
			deleteRelationTagCurrentStatement = statementContainer
					.add(dbCtx.prepareStatement(DELETE_SQL_RELATION_TAG_CURRENT));
			switch (dbCtx.getDatabaseType()) {
			case POSTGRESQL:
				insertRelationMemberStatement = statementContainer
						.add(dbCtx.prepareStatement(INSERT_SQL_RELATION_MEMBER_PGSQL));
				break;
			case MYSQL:
				insertRelationMemberStatement = statementContainer
						.add(dbCtx.prepareStatement(INSERT_SQL_RELATION_MEMBER_MYSQL));
				break;
			default:
			...
			switch (dbCtx.getDatabaseType()) {
			case POSTGRESQL:
				insertRelationMemberCurrentStatement = statementContainer
						.add(dbCtx.prepareStatement(INSERT_SQL_RELATION_MEMBER_CURRENT_PGSQL));
				break;
			case MYSQL:
				insertRelationMemberCurrentStatement = statementContainer
						.add(dbCtx.prepareStatement(INSERT_SQL_RELATION_MEMBER_CURRENT_MYSQL));
				break;
			default:
				throw new OsmosisRuntimeException("Unknown database type " + dbCtx.getDatabaseType() + ".");
			}
			deleteRelationMemberCurrentStatement = statementContainer
					.add(dbCtx.prepareStatement(DELETE_SQL_RELATION_MEMBER_CURRENT));
		}
		...
		// Update the relation if it already exists in the history table, otherwise
		// insert it.
		try {
		...
		if (exists) {
			...
			try {
				...
				throw new OsmosisRuntimeException("Unable to update history relation with id=" + relation.getId() + ".",
						e);
			}
		} else {
			...
			try {
				...
				throw new OsmosisRuntimeException("Unable to insert history relation with id=" + relation.getId() + ".",
						e);
			}
		}
		...
		for (int i = 0; i < relationMemberList.size(); i++) {
			...
			try {
				...
				insertRelationMemberStatement.setString(prmIndex++,
						memberTypeRenderer.render(relationMember.getMemberType()));
		...
		if (populateCurrentTables) {
			...
			// Update the relation if it already exists in the current table, otherwise
			// insert it.
			try {
			...
			if (exists) {
				...
				try {
					...
					updateRelationCurrentStatement.setTimestamp(prmIndex++,
							new Timestamp(relation.getTimestamp().getTime()));
				...
				try {
					...
					insertRelationCurrentStatement.setTimestamp(prmIndex++,
							new Timestamp(relation.getTimestamp().getTime()));
			...
			for (int i = 0; i < relationMemberList.size(); i++) {
				...
				try {
					...
					insertRelationMemberCurrentStatement.setString(prmIndex++,
							memberTypeRenderer.render(relationMember.getMemberType()));
