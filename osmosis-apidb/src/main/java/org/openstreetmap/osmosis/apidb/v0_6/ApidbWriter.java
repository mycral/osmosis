...
public class ApidbWriter implements Sink, EntityProcessor {
	...
	private static final List<String> DISABLE_KEY_TABLES = Arrays.asList(new String[] { "nodes", "node_tags", "ways",
			"way_tags", "way_nodes", "relations", "relation_tags", "relation_members" });
	...
	private static final String LOAD_CURRENT_WAYS = "INSERT INTO current_ways SELECT way_id, changeset_id, timestamp, visible, version FROM ways"
			+ WHERE_WAY_ID_IN_RANGE;

	private static final String LOAD_CURRENT_WAY_TAGS = "INSERT INTO current_way_tags SELECT way_id, k, v FROM way_tags"
			+ WHERE_WAY_ID_IN_RANGE;

	private static final String LOAD_CURRENT_WAY_NODES = "INSERT INTO current_way_nodes SELECT way_id, node_id, sequence_id FROM way_nodes"
			+ WHERE_WAY_ID_IN_RANGE;

	private static final String LOAD_CURRENT_RELATIONS = "INSERT INTO current_relations SELECT relation_id, changeset_id, timestamp, visible, version"
			+ " FROM relations WHERE relation_id >= ? AND relation_id < ?";

	private static final String LOAD_CURRENT_RELATION_TAGS = "INSERT INTO current_relation_tags SELECT relation_id, k, v FROM relation_tags"
			+ " WHERE relation_id >= ? AND relation_id < ?";

	private static final String LOAD_CURRENT_RELATION_MEMBERS = "INSERT INTO current_relation_members (relation_id, member_id, member_role, member_type, sequence_id)"
	...
	private static final int INSERT_BULK_ROW_COUNT_NODE = 3000;
	private static final int INSERT_BULK_ROW_COUNT_NODE_TAG = 4000;
	private static final int INSERT_BULK_ROW_COUNT_WAY = 4000;
	private static final int INSERT_BULK_ROW_COUNT_WAY_TAG = 4000;
	private static final int INSERT_BULK_ROW_COUNT_WAY_NODE = 4000;
	private static final int INSERT_BULK_ROW_COUNT_RELATION = 4000;
	private static final int INSERT_BULK_ROW_COUNT_RELATION_TAG = 4000;
	private static final int INSERT_BULK_ROW_COUNT_RELATION_MEMBER = 4000;
	...
	/**
	 * Sets node reference values as bind variable parameters to a way node insert
	 * query.
	 *
	 * @param statement
	 *            The prepared statement to add the values to.
	...
	/**
	 * Sets member reference values as bind variable parameters to a relation member
	 * insert query.
	 *
	 * @param statement
	 *            The prepared statement to add the values to.
	 ...
	 * statement row count. If complete is true, all remaining rows will be written
	 * using single row insert statements.
	 * 
	 * @param complete
	 *            If true, all data will be written to the database. If false, some
	 *            data may be left until more data is available.
	 */
	...
	/**
	 * Flushes node tags to the database. If complete is false, this will only write
	 * node tags until the remaining node tag count is less than the multi-row
	 * insert statement row count. If complete is true, all remaining rows will be
	...
	/**
	 * Flushes ways to the database. If complete is false, this will only write ways
	 * until the remaining way count is less than the multi-row insert statement row
	 * count. If complete is true, all remaining rows will be written using single
	...
	/**
	 * Flushes way tags to the database. If complete is false, this will only write
	 * way tags until the remaining way tag count is less than the multi-row insert
	 * statement row count. If complete is true, all remaining rows will be written
	 * using single row insert statements.
	 * 
	 * @param complete
	...
	/**
	 * Flushes way nodes to the database. If complete is false, this will only write
	 * way nodes until the remaining way node count is less than the multi-row
	 * insert statement row count. If complete is true, all remaining rows will be
	 * written using single row insert statements.
	 * 
	 * @param complete
	 ...
	 * relations until the remaining way count is less than the multi-row insert
	 * statement row count. If complete is true, all remaining rows will be written
	 * using single row insert statements.
	 * 
	 * @param complete
	 ...
	 * multi-row insert statement row count. If complete is true, all remaining rows
	 * will be written using single row insert statements.
	 * 
	 * @param complete
	 *            If true, all data will be written to the database. If false, some
	 *            data may be left until more data is available.
	 ...
	 * only write relation members until the remaining relation member count is less
	...
	private void addWayTags(Way way) {
		for (Tag tag : way.getTags()) {
			wayTagBuffer
					.add(new DbFeatureHistory<DbFeature<Tag>>(new DbFeature<Tag>(way.getId(), tag), way.getVersion()));
		}
