...
public class RelationDao extends EntityDao<Relation> {
	...
	private ReleasableIterator<DbFeatureHistory<DbOrderedFeature<RelationMember>>> getRelationMemberHistory(
		...
		FileBasedSort<DbFeatureHistory<DbOrderedFeature<RelationMember>>> sortingStore = new FileBasedSort<DbFeatureHistory<DbOrderedFeature<RelationMember>>>(
		...
		try {
			...
			sql = "SELECT rm.relation_id AS id, rm.member_id, rm.member_role, rm.member_type, rm.version, rm.sequence_id"
					+ " FROM " + "relation_members rm" + " INNER JOIN " + selectedEntityStatement
			...
			storeListener = new SortingStoreRowMapperListener<DbFeatureHistory<DbOrderedFeature<RelationMember>>>(
					sortingStore);
	...
	protected List<FeatureHistoryPopulator<Relation, ?, ?>> getFeatureHistoryPopulators(String selectedEntityTableName,
		...
		// Wrap the relation node source into a feature history populator that can
		// attach them to their
		// owning relations.
		featurePopulators.add(new FeatureHistoryPopulator<Relation, RelationMember, DbOrderedFeature<RelationMember>>(
